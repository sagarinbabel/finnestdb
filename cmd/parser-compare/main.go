...
// by dataset name. Multiple files for the same dataset name resolve to the
// most-recent one by mtime (latest wins) — supports the convention of
// timestamped baseline filenames.
...
func emitBeforeAfterTable(now []*eval.Report, prev map[string]*eval.Report, mainParser string, bootstrap int, rng *rand.Rand) {
	...
	fmt.Println()
	fmt.Println("| Dataset | Cases | Metric | custom-prev | custom-now | Δ | analyzer |")
	fmt.Println("|---|---:|---|---:|---:|---:|---:|")
