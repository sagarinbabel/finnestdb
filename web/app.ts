...
async function postKnownWords(words: string[], source: 'manual' | 'anki' = 'manual', signal?: AbortSignal): Promise<KnownWordsImportResponse> {
...
async function putKnownWords(words: string[], scope: 'anki' | 'all' = 'anki', signal?: AbortSignal): Promise<KnownWordsReplaceResponse> {
    const resp = await fetch('/api/known-words', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
...
async function runAnkiImport(): Promise<void> {
    ...
    try {
        ...
        if (ankiImport.replaceMode) {
            ...
            const data = await putKnownWords(words, scope, abortController.signal);
            if (isAnkiImportCancelled(sessionID)) return;
            renderKnownWordsUnresolved(data.unresolved || []);
            ...
            // own source.
            const data = await postKnownWords(words, 'anki', abortController.signal);
            if (isAnkiImportCancelled(sessionID)) return;
            renderKnownWordsUnresolved(data.unresolved || []);
            if (msg) msg.textContent = 'Done.';
