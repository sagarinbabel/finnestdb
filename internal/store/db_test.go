...
func TestAcceptedParseFeedbackOverridesLexOverlaySurface(t *testing.T) {
	db := newTestDB(t)
	user := createTestUser(t, db, "feedback-overlay-user@example.com")
	admin := createTestUser(t, db, "feedback-overlay-admin@example.com")

	before := db.BatchLookupForms([]string{"varsin"}, "FI", "custom")["varsin"]
	if before.Lemma != "varsin" || before.POS != "ADV" {
		t.Fatalf("before acceptance got %s/%s, want overlay varsin/ADV", before.Lemma, before.POS)
	}

	parseID, err := db.CreateParseSession(&user.ID, "FI", "custom", "varsin", 1, 1)
	if err != nil {
		t.Fatalf("CreateParseSession: %v", err)
	}
	feedbackID, err := db.CreateParseFeedback(ParseFeedback{
	...
	if err := db.ReviewParseFeedback(feedbackID, admin.ID, "accepted", "override overlay"); err != nil {
		t.Fatalf("ReviewParseFeedback: %v", err)
	}

	after := db.BatchLookupForms([]string{"varsin"}, "FI", "custom")["varsin"]
	if after.Lemma != "varsi" || after.POS != "NOUN" {
		t.Fatalf("after acceptance got %s/%s, want custom override varsi/NOUN", after.Lemma, after.POS)
	}
	allAfter := db.BatchLookupAllForms([]string{"varsin"}, "FI", "custom")["varsin"]
	if len(allAfter) != 1 || allAfter[0].Lemma != "varsi" || allAfter[0].POS != "NOUN" {
		t.Fatalf("all forms after acceptance got %+v, want only varsi/NOUN", allAfter)
	}
}
