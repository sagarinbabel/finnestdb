...
| Parser evaluation baseline | `parser-baseline-2026-05-12-a-T1526Z` (latest; see "Parser evaluation baseline history" below) | First freeze on `parser-v2` / `2026.05.12a`, captures the 2026-05-12 PR cascade end-to-end (#183/#185/#187/#188/#189/#191/#193/#195). Full history of baselines is preserved in the dedicated section below this table and as a row-per-event trend in [`PARSER_EVOLUTION.md`](PARSER_EVOLUTION.md). The wall-clock `THHMMZ` suffix is part of the baseline ID — multiple baselines on the same parser-behavior version are distinguished by run-start UTC time. |
