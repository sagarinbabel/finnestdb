...
- ~~The `omorfi` adapter dispatch does not auto-discover a `.venv-omorfi/bin/python` sibling the way the `estnltk` adapter discovers `.venv-estnltk/bin/python`.~~ Resolved 2026-05-07: `runExternalOmorfi` now lives in `internal/evalparsers` and does the same `findSiblingVenvPython(path, ".venv-omorfi")` lookup, and `make setup-omorfi` was rewritten to create the venv locally (system Python on macOS Homebrew enforces PEP 668, so the prior system-pip path failed there).
