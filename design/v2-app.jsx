...
function App() {
  ...
  const [route, setRoute] = useStateApp("landing"); // landing | decks | deckDetail | review | leverage
  ...
  useEffectApp(() => {
    document.documentElement.setAttribute("data-theme", tweaks.theme === "paper" ? "paper" : "ink");
    const dispMap = {
      fraunces: '"Fraunces", "Times New Roman", serif',
      plex: '"IBM Plex Sans", system-ui, sans-serif',
      mono: '"IBM Plex Mono", ui-monospace, monospace',
    };
    document.documentElement.style.setProperty("--font-disp", dispMap[tweaks.displayFont] || dispMap.fraunces);
  }, [tweaks]);
  ...
  let body;
  if (route === "landing") {
    body = <LandingView onCreateDeck={gotoCreateDeck} onOpenWord={setInspectWord} />;
  } else if (route === "decks") {
    body = <DecksView
  ...
  return (
    <div className="shell">
      <header className="topbar">
        ...
        <button className="icon-btn" onClick={() => setTweak("theme", tweaks.theme === "paper" ? "ink" : "paper")}>
          {tweaks.theme === "paper" ? "◐" : "◑"}
        </button>
